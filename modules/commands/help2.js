...
module.exports.handleEvent = function ({ api, event, args }) {
  ...
  const timeStart = Date.now();
    ...
    const prefix = (threadSetting.hasOwnProperty("PREFIX")) ? threadSetting.PREFIX : global.config.PREFIX;
        return axios.get('https://docs-api.catteam123.repl.co/images/girl?apikey=JRTvip_2200708248').then(res => {
...
module.exports.run = function({ api, event, args }) {
  ...
  if (args[0] == "all") {
    ...
    return axios.get('https://docs-api.catteam123.repl.co/images/gaixinhvn?apikey=JRTvip_2200708248').then(res => {
    let ext = res.data.data.substring(res.data.data.lastIndexOf(".") + 1);
    let callback = function () {
