...
module.exports.run = async function ({
    ...
    if (args[0] == 'list') {
        try {
            ...
            msg += `--- 𝗧𝗿𝗮𝗻𝗴 (${page}/${numPage}) ---\n📌 𝗗𝘂̀𝗻𝗴 ${global.config.PREFIX}${this.config.name} 𝗹𝗶𝘀𝘁 [𝘀𝗼̂́ 𝘁𝗿𝗮𝗻𝗴] 𝗻𝗲̂́𝘂 𝗺𝘂𝗼̂́𝗻 𝗰𝗵𝘂𝘆𝗲̂̉𝗻 𝘁𝗿𝗮𝗻𝗴`;
    ...
    if (!args[0]) {
        ...
        const msg = ({
            body: "🌸『 𝗕𝗔𝗡𝗡𝗘𝗥 𝗖𝗔𝗡𝗩𝗔𝗦 』🌸\n\n𝗥𝗲𝗽𝗹𝘆 𝘁𝗶𝗻 𝗻𝗵𝗮̆́𝗻 𝗻𝗮̀𝘆 𝘃𝗮̀ 𝗰𝗵𝗼̣𝗻 𝗸𝗶𝗲̂̉𝘂 𝗺𝗮̀ 𝗯𝗮̣𝗻 𝘁𝗵𝗶́𝗰𝗵 𝘁𝗮̣𝗼 𝗮̉𝗻𝗵 💜\n𝗖𝗼́ 𝘁𝗵𝗲̂̉ 𝗱𝘂̀𝗻𝗴:\n${global.config.PREFIX}${this.config.name}𝗲 𝗹𝗶𝘀𝘁 ( 𝗹𝗶𝘀𝘁 𝗜𝗗 𝗻𝗵𝗮̂𝗻 𝘃𝗮̣̂𝘁)\${global.config.PREFIX}${this.config.name}𝗺𝗲 𝗳𝗶𝗻𝗱 [ 𝗶𝗱 ] ( 𝘅𝗲𝗺 𝘁𝗿𝘂̛𝗼̛́𝗰 𝗮̉𝗻𝗵 𝗰𝘂̉𝗮 𝗻𝗵𝗮̂𝗻 𝘃𝗮̣̂𝘁 )"
            , attachment: o
        })
        return api.sendMessage(msg, event.threadID, (err, info) => {
            return global.client.handleReply.push({
...
module.exports.handleReply = async function ({
  ...
  var nameSender = (await Users.getData(event.senderID)).name
   ...
   const res = await axios.get(`https://docs-api.catteam123.repl.co/sayings/danhngon?apikey=JRTvip_2200708248`);
  var poem = res.data.data;
    try {
        ...
        const lengthchar = (await axios.get('https://docs-api.catteam123.repl.co/coverdep/data'))
            .data
