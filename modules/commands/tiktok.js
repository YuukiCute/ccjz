...
module.exports.run = async ({ api, event, args }) => {
  ...
  if (args[0] == 'post') return runListUserPost(api, event, (await get(`${global.configApi.domain}/tiktok/postuserv2?unique_id=${args[1]}`)).data.data.videos, 6, 1, true, event.senderID);
  ...
  switch (type.toLowerCase()) {
    ...
    case "info":
      ...
      try {
        axios.get(encodeURI(`${global.configApi.domain}/tiktok/infov2?username=${keyword}`)).then(async (res) => {
    ...
    case 'search':
      ...
      axios.get(`${global.configApi.domain}/tiktok/searchvideo?keywords=${encodeURI(search)}`).then(async res => {
    ...
    case "video":
      try {   
        const res = await axios.get(`${global.configApi.domain}/tiktok/downloadvideo?url=${keyword}`);
    ...
    case "-tr":
    case "trending":
      axios.get(`${global.configApi.domain}/tiktok/trendingtiktok`).then(response_api => {
