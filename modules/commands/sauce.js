...
var config = {
	...
	usages: "souce + reply ảnh",
	cooldowns: 3
},
	run = async ({ api, event, args }) => {
		if ("message_reply" != event.type || event.messageReply.attachments.length > 1) return api.sendMessage("[⚜️] ➜ Vui lòng reply vào bức ảnh bạn cần tìm kiếm!", event.threadID, event.messageID);
		var arr = [],
			image = [];
