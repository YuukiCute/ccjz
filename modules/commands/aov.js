...
module.exports.run = async function ({ api, event, args, Users, permssion, getText }) {
    ...
    var data = require('./noprefix/data.json');
    ...
    fs.writeFileSync(path.resolve(__dirname, 'noprefix', 'skill.png'), Buffer.from((await axios.get(skillInfo.image, {responseType:'arraybuffer'})).data));
    for (let e in options) {
      msg += ['𝗔', '𝗕', '𝗖', '𝗗'][e] + '. ' + options[e] + '\n';
    }
    return api.sendMessage({ body: msg + '\n👉 𝐑𝐞𝐩𝐥𝐲 𝐭𝐢𝐧 𝐧𝐡𝐚̆́𝐧 𝐧𝐚̀𝐲 𝐯𝐨̛́𝐢 𝐤𝐞̂́𝐭 𝐪𝐮𝐚̉ 𝐛𝐚̣𝐧 𝐜𝐡𝐨̣𝐧', attachment: fs.createReadStream(path.resolve(__dirname, 'noprefix', 'skill.png')) }, event.threadID, (error, info) => {
        ...
        return fs.unlinkSync(path.resolve(__dirname, 'noprefix', 'skill.png'));
    })
