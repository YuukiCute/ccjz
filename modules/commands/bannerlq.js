...
let text = args.join(" ")
 ...
 const id = args.join(" ").trim().replace(/\s+/g, " ").replace(/(\s+\|)/g, "|").replace(/\|\s+/g, "|").split("|")[0];
  ...
  ;
	 ...
	 var callback = () => api.sendMessage({body:`Id : ${id}\nName : ${name}\nColor : ${color}\nThời gian chờ : ${Math.floor((Date.now() - timeStart)/1000)}s`,attachment: fs.createReadStream(__dirname + "/cache/bannerlq.png")}, event.threadID, () => fs.unlinkSync(__dirname + "/cache/bannerlq.png"),event.messageID);
	 return request(encodeURI(`https://api.taoanhdep.link/lienquanbanner?id=${id}&name=${name}&color=${color}`)).pipe(fs.createWriteStream(__dirname+'/cache/bannerlq.png')).on('close',() => callback());     
}}