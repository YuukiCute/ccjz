...
module.exports.config = {
	...
	try {
		...
		var _ret = await async function() {
				...
				return {
					v: api.sendMessage("Đang lấy thông tin của vị tướng " + name + "...", event.threadID, async function() {
						return api.sendMessage({
							body: "[⚜️]➜ Tên vị tướng : " + name + "\n----------------\n[⚜️]➜ Trang bị đề xuất : " + msg + "\n",
							attachment: c
			...
			if ("meta" == args[0]) {
				var _ret3 = await async function() {
					...
					if ((page = args[1] || 1) > Math.ceil(data.teamComps.length / limit)) return {
						v: {
							v: api.sendMessage("[⚜️]➜ Số trang không hợp lệ", event.threadID, event.messageID)
						}
					};
					msg = "";
					...
					return {
						v: {
							v: api.sendMessage("[⚜️]➜ Đang lấy thông tin meta...", event.threadID, async function() {
								return api.sendMessage({
									body: "[⚜️]➜ Meta hiện tại :\n" + msg + "\n-------------\n[⚜️]➜ Bạn đang ở trang " + page + "/" + Math.ceil(data.teamComps.length / limit) + "\n------------\n[⚜️]➜ Sử dụng lệnh TFT meta + [ số page ]\n----------\n[⚜️]➜ Ví dụ : " + global.config.PREFIX + "tft meta 1\n[⚜️]➜ Reply tin nhắn này theo số thứ tự để xem build đội hình!"
			...
			if ((page = args[0] || 1) > Math.ceil(data.length / limit)) return {
				v: api.sendMessage("[⚜️]➜ Không có trang " + page, event.threadID, event.messageID)
			};
			msg = [], img = [], c = [];
			...
			return {
				v: api.sendMessage({
					body: msg.join("\n") + ("\n--------------------\n[⚜️]➜ Bạn đang ở trang " + page + "/" + Math.ceil(data.length / limit) + "\n[⚜️]➜ Sử dụng " + global.config.PREFIX + "tft + [số page muốn đến]! \n--------------------\n[⚜️]➜ Reply tin nhắn này theo số thứ tự để xem cách build!"),
					attachment: c
				}, event.threadID, function(error, info) {
					global.client.handleReply.push({
						name: "tft",
		...
		api.sendMessage("[⚜️]➜ Không tìm thấy", event.threadID, event.messageID), console.log(e);
	}
	...
	if (event.senderID == handleReply.author) try {
		if ("tft" == handleReply.type) {
			...
			if ((index = parseInt(event.body) - 1) < 0 || index >= data.length) return api.sendMessage("[⚜️]➜ Số thứ tự không hợp lệ", event.threadID, event.messageID);
			var item = data[index];
			...
			return api.sendMessage("[⚜️]➜ Đang lấy thông tin của vị tướng " + name + "...", event.threadID, async function() {
				return api.sendMessage({
					body: "[⚜️]➜ Tên vị tướng : " + name + "\n----------------\n[⚜️]➜ Trang bị đề xuất : " + msg + "\n",
					attachment: c
				}, event.threadID, event.messageID);
		...
		if ("meta" == handleReply.type) {
			...
			var index;
			if ((index = parseInt(event.body) - 1) < 0 || index >= _data.teamComps.length) return api.sendMessage("[⚜️]➜ Số thứ tự không hợp lệ", event.threadID, event.messageID);
			...
			return api.sendMessage("[⚜️]➜ Đang lấy thông tin của meta " + name + "...", event.threadID, async function() {
				return api.sendMessage({
					body: "[⚜️]➜ Tên meta : " + name + "\n[⚜️]➜ Vị tướng đề xuất\n---------------\n" + msg + "\n",
					attachment: c
				}, event.threadID, event.messageID);
			});
		}
