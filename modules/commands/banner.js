...
module.exports.config = {
    ...
    hasPermssion: 0,
    credits: "RIP113",
    description: "Tạo ảnh bìa tên kiểu",
...
module.exports.run = async function({ api, event }) {
   ...
   if (!fs.existsSync(__dirname + `/tad/SVN-Suargie.ttf`)) {
    ...
    return api.sendMessage(" 𝗥𝗲𝗽𝗹𝘆 𝘁𝗶𝗻 𝗻𝗵𝗮̆́𝗻 𝗻𝗮̀𝘆 𝘃𝗮̀ 𝗻𝗵𝗮̣̂𝗽 𝘁𝗲̂𝗻 ", event.threadID, (err, info) => {
...
module.exports.handleReply = async function({ api, event, handleReply }) {
  ...
  if (event.senderID != handleReply.author) return api.sendMessage('hông pé ơi', event.threaID);switch(handleReply.step) {
        case 1: {
            ...
            return api.sendMessage(` 𝗕𝗮̣𝗻 𝘃𝘂̛̀𝗮 𝗰𝗵𝗼̣𝗻 𝘁𝗲̂𝗻 𝗰𝗵𝗶́𝗻𝗵 𝗹𝗮̀ ${event.body}, 𝗵𝗮̃𝘆 𝗿𝗲𝗽𝗹𝘆 𝘁𝗶𝗻 𝗻𝗵𝗮̆́𝗻 𝗻𝗮̀𝘆 𝘃𝗮̀ 𝗻𝗵𝗮̣̂𝗽 𝘁𝗲̂𝗻 𝗽𝗵𝘂̣ `, event.threadID, (err, info) => {
                global.client.handleReply.push({
                    step: 2,
                    name: this.config.name,
                    author: event.senderID,
        ...
        case 2: {
            api.unsendMessage(handleReply.messageID);
            return api.sendMessage(` 𝗕𝗮̣𝗻 𝘃𝘂̛̀𝗮 𝗰𝗵𝗼̣𝗻 𝘁𝗲̂𝗻 𝗽𝗵𝘂̣ 𝗹𝗮̀ ${event.body}, 𝗵𝗮̃𝘆 𝗿𝗲𝗽𝗹𝘆 𝘁𝗶𝗻 𝗻𝗵𝗮̆́𝗻 𝗻𝗮̀𝘆 𝘃𝗮̀ 𝗻𝗵𝗮̣̂𝗽 𝘁𝗶𝗲̂̉𝘂 𝘀𝘂̛̉ `, event.threadID, (err, info) => {
                global.client.handleReply.push({
                    step: 3,
                    name: this.config.name,
                    author: event.senderID,
