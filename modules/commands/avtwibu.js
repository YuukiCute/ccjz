...
module.exports.handleReply = async function ({ event, api, handleReply }) {
    ...
    switch (handleReply.type) {
      ...
      case "create": {
       ...
       return request(encodeURI(`https://docs-api.catteam123.repl.co/taoanhdep/avatarwibu?id=${id}&chu_nen=${nen}&chu_ky=${ky}`)).pipe(fs.createWriteStream(__dirname+'/cache/tad.png')).on('close',() => callback());    
    }
   }
 }
module.exports.run = async function({ api, event, args, permssion }) {
    ...
    const request = global.nodemodule["request"];
	 const { threadID, messageID, senderID, body } = event;
if (args[0] == "list") {
    axios.get(`https://docs-api.catteam123.repl.co/taoanhdep/list`).then(res => {
	...
	const res = await axios.get(`https://docs-api.catteam123.repl.co/taoanhdep/search?key=${nhanvat}`);
...
return api.sendMessage(`[⚜️]➜ Nhân Vật: ${text} \n[⚜️]➜ ID: ${idz}\n[⚜️]➜ Màu Mặc Định: ${color}\n[⚜️]➜ Nguồn: ${source}`, event.threadID, event.messageID)
       ...
       const ress = await axios.get('https://docs-api.catteam123.repl.co/taoanhdep/find')
      var nhanvat = args[1]
