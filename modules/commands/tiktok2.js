...
try {
  ...
  case "video":
    case "v":{
       const link = args[1];
      if (!link) return api.sendMessage("[⚜️]➜ Cần link video tiktok", threadID);
...
const hastag = str.split(' ').filter(i => i.startsWith('#')).join(', ');
       var callback = () => api.sendMessage({body:`[⚜️]=== 『 VIDEO TIKTOK 』 ===[⚜️]\n◆━━━━━━━━━━━━━━━━◆\n\n[⚜️]➜ Tên kênh: ${res.data.data.author.nickname}\n[⚜️]➜ ID: ${res.data.data.author.unique_id}\n[⚜️]➜ Lượt Tim: ${res.data.data.digg_count}\n[⚜️]➜ Hastag: ${hastag}\n[⚜️]➜ Lượt Bình Luận: ${res.data.data.comment_count}\n[⚜️]➜ Lượt Chia Sẻ: ${res.data.data.share_count}\n[⚜️]➜ Lượt Tải Xuống: ${res.data.data.download_count}`,attachment: fs.createReadStream(__dirname + "/cache/tkvd.mp4")}, event.threadID, () => fs.unlinkSync(__dirname + "/cache/tkvd.mp4"),event.messageID);
...
var url = res.data.data.music;
   ...
   return request(encodeURI(`${url}`)).pipe(fs.createWriteStream(__dirname+'/cache/tkvd.mp3')).on('close',() => callback());  
    ...
    case "i":{
      ...
      if (!username) return api.sendMessage("[⚜️]➜ Thiếu ID/USER TikTok", threadID);
...
default: api.sendMessage({body: `[⚜️]=== 『 TIKTOK 』 ===[⚜️]\n◆━━━━━━━━━━━━━━━━◆\n\n[⚜️]➜ video + url ( video/v)
[⚜️]➜ music +url (music/audio/a/m)
[⚜️]➜ info + user/id ( i/info)
[⚜️]➜ search + keyword ( search/s)`,
...
module.exports.handleReply = async function({ handleReply, api, event }){
  const { threadID, messageID, senderID, body } = event;
  if (senderID != handleReply.author) return api.sendMessage(`Bạn không đủ quyền hạn để làm điều này!`, threadID);
   const {
       play, title, region
   } = handleReply.Dungkon[event.args[0]-1];
   const dest = __dirname + '/cache/search_vd.mp4'; 
   api.sendMessage({body:`[⚜️]➜ Tải video\n[⚜️]➜ Tiêu đề: ${title}\n[⚜️]➜ Quốc gia: ${region}`, attachment: await download(play, dest)}, threadID);
};
