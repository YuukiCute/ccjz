...
module.exports.run = function({ api, event, args }) {
	...
	if (!content) return api.sendMessage("[⚜️]→ Sai Format", threadID, messageID);
	if (content.indexOf(`del`) == 0) {
		let delThis = content.slice(4, content.length);
		if (!delThis) return api.sendMessage("[⚜️]→ Không tìm thấy shortcut bạn cần xóa", threadID, messageID);
		return readFile(__dirname + "/cache/shortcut.json", "utf-8", (err, data) => {
			if (err) throw err;
			var oldData = JSON.parse(data);
			var getThread = oldData.find(item => item.id == threadID).shorts;
			if (!getThread.some(item => item.in == delThis)) return api.sendMessage("[⚜️]→ Không tìm thấy shortcut bạn cần xóa", threadID, messageID);
			getThread.splice(getThread.findIndex(item => item.in === delThis), 1);
			writeFile(__dirname + "/cache/shortcut.json", JSON.stringify(oldData), "utf-8", (err) => (err) ? console.error(err) : api.sendMessage("[⚜️]→ Đã xóa shortcut thành công!", threadID, messageID));
		});
	}
		...
		return readFile(__dirname + "/cache/shortcut.json", "utf-8", (err, data) => {
			...
			let msg = '';
			if (!allData.some(item => item.id == threadID)) return api.sendMessage("[⚜️]→ Hiện tại không có shortcut nào", threadID, messageID);
			...
			if (!msg) return api.sendMessage("[⚜️]→ Hiện tại không có shortcut nào", threadID, messageID);
			api.sendMessage("[⚜️]→ Sau đây là shortcuts có trong nhóm: " + msg, threadID, messageID);
		});
	else {
		let narrow = content.indexOf(" => ");
		if (narrow == -1) return api.sendMessage("[⚜️]→ Sai Format", threadID, messageID);
		let shortin = content.slice(0, narrow);
		let shortout = content.slice(narrow + 4, content.length);
		if (shortin == shortout) return api.sendMessage("[⚜️]→ 2 input và output giống nhau", threadID, messageID);
		...
		return readFile(__dirname + "/cache/shortcut.json", "utf-8", (err, data) => {
			...
			if (!oldData.some(item => item.id == threadID)) {
				...
				return writeFile(__dirname + "/cache/shortcut.json", JSON.stringify(oldData), "utf-8", (err) => (err) ? console.error(err) : api.sendMessage("[⚜️]→ Tạo shortcut thành công", threadID, messageID));
			}
			else {
				...
				if (getShort.shorts.some(item => item.in == shortin)) {
					...
					api.sendMessage("[⚜️]→ Shortcut đã tồn tại trong group này", threadID, messageID);
