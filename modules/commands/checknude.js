...
if (typeof data.checkNude == "undefined" || data.checkNude == false) return;
  ...
  const ress = await axios.get(`${global.configApi.domain}/imgurupload?link=${encodeURIComponent(dataa.url)}`)
  const link = ress.data.uploaded.image
  var res = await axios.get(`${global.configApi.domain}/checknude?key=TyRTWWdguv&link=${link}`);
...
const ress = await axios.get(`${global.configApi.domain}/imgurupload?link=${encodeURIComponent(event.messageReply.attachments[0].url)}`)
  const link = ress.data.uploaded.image
  var res = await axios.get(`${global.configApi.domain}/checknude?key=TyRTWWdguv&link=${link}`);
  var checkIMG = res.data.data
