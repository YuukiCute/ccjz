...
module.exports.handleReply = async function({ api, event, handleReply }) {
	...
	try {
		...
		ytdl(handleReply.link[event.body - 1])
			.pipe(createWriteStream(__dirname + `/cache/audio/${handleReply.link[event.body - 1]}.m4a`))
			.on("close", () => {
  ...
  //if (global.client.sing == true) return api.sendMessage("Hệ thống đang xử lý yêu cầu từ box khác, vui lòng quay lại sau", event.threadID, event.messageID);
	...
	if (urlValid) {
		try {
			...
			ytdl(args[0])
				.pipe(createWriteStream(__dirname + `/cache/audio/${id}.m4a`))
				.on("close", () => {
