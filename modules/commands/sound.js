...
const handleReply = async ({ api, event, handleReply }) => {
    try {
        ...
        if(fs.statSync(data).size > 26214400) return api.sendMessage('[⚜️]➜ Không thể gửi file vì dung lượng lớn hơn 25MB.', event.threadID, () => fs.unlinkSync(path), event.messageID);
        api.unsendMessage(handleReply.messageID);
        const message = {
            body: `======「 𝗠𝗨𝗦𝗜𝗖 」======\n\n[⚜️]➜ Tiêu đề: ${info.title}\n[⚜️]➜ Tên kênh: ${info.author}\n[⚜️]➜ Ngày tải lên: ${info.publishDate}\n[⚜️]➜ Thời lượng: ${convertHMS(info.dur)}\n[⚜️]➜ Lượt xem: ${info.viewCount}\n[⚜️]➜ Lượt thích: ${info.likes}\n[⚜️]➜ Thời gian xử lí: ${Math.floor((Date.now() - info.timestart) / 1000)} giây\n[⚜️]➜ Link tải: https://www.y2meta.com/vi/youtube/${handleReply.link[event.body -1]}\n\n======= ${time} =======`,
...
const run = async function ({ api, event, args }) {
    ...
    if (args[0]?.startsWith("https://")) {
        try {
            ...
            if (fs.statSync(data).size > 26214400) { return api.sendMessage('[⚜️]➜ Không thể gửi file vì dung lượng lớn hơn 25MB.', event.threadID, () => fs.unlinkSync(data), event.messageID); }
