...
const time = process.uptime();
  ...
  const res = await axios.get(`https://api.popcat.xyz/weather?q=${encodeURI('Hồ Chí Minh')}`);
    var abc =`[⚜️]=== 『 NOTIFICATION WEATHER 』 ===[⚜️]\n◆━━━━━━━━━━━━━━━━◆\n\n[⚜️]➜ 𝗰𝗮̣̂𝗽 𝗻𝗵𝗮̣̂𝘁 𝘁𝗵𝗼̛̀𝗶 𝘁𝗶𝗲̂́𝘁 𝘁𝗮̣𝗶 ${res.data[0].location.name}\n[⚜️]➜ 𝗩𝗮̀𝗼 𝗹𝘂́𝗰: ${res.data[0].current.day} ${res.data[0].current.date}\n[⚜️]➜ 𝗡𝗵𝗶𝗲̣̂𝘁 đ𝗼̣̂: ${res.data[0].current.temperature}°${res.data[0].location.degreetype}\n[⚜️]➜ 𝗠𝗼̂ 𝘁𝗮̉: ${res.data[0].current.skytext}\n[⚜️]➜ đ𝗼̣̂ 𝗮̂̉𝗺: ${res.data[0].current.humidity}\n[⚜️]➜ 𝗛𝘂̛𝗼̛́𝗻𝗴 𝗴𝗶𝗼́: ${res.data[0].current.winddisplay}\n[⚜️]➜ 𝗚𝗵𝗶 𝗻𝗵𝗮̣̂𝗻 𝘃𝗮̀𝗼 𝗹𝘂́𝗰: ${res.data[0].current.observationtime} \n\n[⚜️]➜ 𝐍𝐡𝐚̣̂𝐩 ${global.config.PREFIX}weather 𝐭𝐢̉𝐧𝐡/𝐭𝐡𝐚̀𝐧𝐡 𝐩𝐡𝐨̂́ đ𝐞̂̉ 𝐱𝐞𝐦 𝐭𝐡𝐨̛̀𝐢 𝐭𝐢𝐞̂́𝐭 𝐧𝐡𝐞́!!!!`;
    msg = msg.replace(/{abc}/,abc);
msg = msg.replace(/{hours}/g, hours)
  msg = msg.replace(/{minutes}/g, minutes)
  msg = msg.replace(/{seconds}/g, seconds)
    msg = msg.replace(/{time}/g, require("moment-timezone").tz("Asia/Ho_Chi_Minh").format("HH:mm:ss (D/MM/YYYY) (dddd)")).replace(/{thinh}/g, (await axios.get(`https://docs-api.catteam123.repl.co/saying/hearing?apikey=JRTvip_2200708248`)).data.data)
            msg = {
                body: msg, attachment: (await axios.get((await axios.get(`https://docs-api.catteam123.repl.co/images/girl?apikey=JRTvip_2200708248`)).data.data, {
                    responseType: 'stream'
                })).data
            };
   global.data.allThreadID.forEach(i => o.api.sendMessage(msg, i));
