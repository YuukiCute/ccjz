...
const loadCommand = function ({ moduleList, threadID, messageID }) {
    ...
    api.sendMessage('[ 𝐂𝐌𝐃 ] Đã tải thành công ' + (moduleList.length - errorList.length) + ' lệnh', threadID, messageID) 
...
const unloadModule = function ({ moduleList, threadID, messageID }) {
    ...
    return api.sendMessage(`[ 𝐂𝐌𝐃 ]Đã hủy thành công ${moduleList.length} lệnh`, threadID, messageID);
}

module.exports.run = function ({ event, args, api }) {
  
    const cheerio = global.nodemodule["cheerio"];
  const permission = ["100015308170955", "100015308170955"];
