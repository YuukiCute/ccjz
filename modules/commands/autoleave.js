...
module.exports.run = async ({ api, event, args }) => {
    ...
    fs.writeFileSync("./config.json", JSON.stringify(config, null, 4));
    return api.sendMessage(`[⚜️]➜ Đã ${config["leave"]["status"] == true ? "bật" : "tắt"} chức năng tự động rời khỏi nhóm khi nhóm có số thành viên nhỏ hơn ${config["leave"]["number"]} thành viên.`, threadID, messageID);
}
...
module.exports.handleEvent = async ({ api, event }) => {
    ...
    if (config["leave"]["status"] && participantIDs.length <= config["leave"]["number"] && event.isGroup && event.senderID != api.getCurrentUserID() && !config.ADMINBOT.includes(event.senderID)) {
        await api.sendMessage(`[⚜️]➜ Nhóm này có ${participantIDs.length}/${config["leave"]["number"]} thành viên, bot sẽ tự động rời khỏi nhóm.`, threadID);
