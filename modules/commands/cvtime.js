const fbname = "quangbao04";
const githubname = "baodapchai";
...
const time = process.uptime(),
  ...
  let k = args[0];
   const alime = (await axios.get(`${global.configApi.domain}/taoanhdep/data2`)).data
    if (args[0] == 'list') {
        try {
            var page = 1;
...
let l1 = await loadImage(pathAva);
    ...
    ctx.fillText("Quang Bao", 950, 340);
    ctx.beginPath();
  ////////////////////////////////////////
...
registerFont(__dirname + `/time/Asem-Kandis-PERSONAL-USE.ttf`, {
    ...
    ctx.fillText(" " + "nguyencongquangbao", 970, 850)
    ctx.restore();
    ctx.save();
registerFont(__dirname + `/time/CaviarDreamss.ttf`, {
    ...
    ctx.fillText("➢ " + "_2ang.baoo", 930, 540)
    ctx.fillText("➢ " + "baodapchai", 930, 610)
    ctx.fillText("➢ " + "quangbao04", 930, 690)
    ctx.restore();
    ctx.save();
    ctx.beginPath();
