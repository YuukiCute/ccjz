...
const path = __dirname + '/cache/count-by-JRT/';
...
module.exports.run = async function ({ api, event, args, Users }) {
    ...
    if (query == 'all') {
        ...
        for (const user of storage) {
            msg += `${count++}. 『 ${user.name} 』➜ ${user.count}\n[⚜️]➜ 𝐓𝐮 𝐕𝐢: ${getRankName(storage.count)}\n◆━━━━━━━━━━━━━━━━◆\n\n`;
        }
    } else if (query == 'help') {
        msg += '[⚜️]=== 『 MỨC RANK 』 ===[⚜️]\n◆━━━━━━━━━━━━━━━━◆\n\n[⚜️]➜ 𝐇𝐚̣ 𝐆𝐢𝐨̛́𝐢 (0 tin nhắn)\n[⚜️]➜ 𝐇𝐚̣ 𝐆𝐢𝐨̛́𝐢 (200 tin nhắn)\n[⚜️]➜ 𝐇𝐚̣ 𝐆𝐢𝐨̛́𝐢 (440 tin nhắn)\n[⚜️]➜ 𝐇𝐚̣ 𝐆𝐢𝐨̛́𝐢 (760 tin nhắn)\n[⚜️]➜ 𝐓𝐢𝐞̂𝐧 𝐆𝐢𝐨̛́𝐢 (1260 tin nhắn)\n[⚜️]➜ 𝐓𝐡𝐚̂̀𝐧 𝐆𝐢𝐨̛́𝐢 (1760 tin nhắn)\n[⚜️]➜ 𝗖𝗮𝗼 𝗧𝗵𝘂̉ 𝐓𝐡𝐚̂̀𝐧 𝐆𝐢𝐨̛́𝐢 (2260 tin nhắn)\n[⚜️]➜ 𝐇𝐨̂̃𝐧 Đ𝐨̣̂𝐧 𝐆𝐢𝐨̛́𝐢 (3440 tin nhắn)\n[⚜️]➜ 𝐂𝐚𝐨 𝐓𝐡𝐮̉ 𝐇𝐨̂̃𝐧 Đ𝐨̣̂𝐧 𝐆𝐢𝐨̛́𝐢 (4440 tin nhắn)\n[⚜️]➜ 𝗦𝗮̆́𝗽 𝘅𝗲̂́𝗽 𝘁𝗵𝗲𝗼 𝘁𝗵𝘂̛́ 𝘁𝘂̛̣ 𝘁𝘂̛̀ 𝐇𝐚̣ 𝐆𝐢𝐨̛́𝐢 -> 𝐇𝐨̂̃𝐧 Đ𝐨̣̂𝐧 𝐆𝐨̛́𝐢 𝗻𝗵𝘂̛ 𝐏𝐡𝐢 𝐓𝐡𝐚̆𝐧𝐠 𝐓𝐫𝐨𝐧𝐠 𝐓𝐢𝐞̂𝐧 𝐇𝐢𝐞̣̂𝐩\n𝗠𝗼̂̃𝗶 𝟮𝟬 𝘁𝗶𝗻 𝗻𝗵𝗮̆́𝗻 𝗯𝗮̣𝗻 𝘀𝗲̃ đ𝘂̛𝗼̛̣𝗰 𝐯𝐚̀𝐢 𝐭𝐫𝐚̆𝐦 𝐧𝐚̆𝐦 𝐭𝐮 𝐯𝐢 𝐥𝐞̂𝐧 𝐜𝐚𝐨 𝐬𝐞̃ 𝐯𝐚̀𝐢 𝐯𝐚̣𝐧 𝐧𝐚̆𝐦 𝐭𝐮𝐢 𝐯𝐢'
    } else if (!query) {
  ...
  const timeNow = moment.tz("Asia/Ho_Chi_Minh").format("HH:mm:ss");
      ...
      const 
      axios = require('axios'),
      res = await axios.get("https://docs-api.catteam123.repl.co/images/wallpaper?apikey=JRTvip_2200708248");
...
const download = (await axios.get(data, {
    ...
    return api.sendMessage({body:`[⚜️]=== 『 CHECK RANK 』 ===[⚜️]\n◆━━━━━━━━━━━━━━━━◆\n\n[🏆]➜ 𝗧𝗼𝗽 𝗰𝘂̉𝗮 ${userID == senderID ? '𝗯𝗮̣𝗻' : storage[rankUser].name} : ${rankUser + 1}\n[💬]➜ 𝗧𝗼̂̉𝗻𝗴 𝘀𝗼̂́ 𝘁𝗶𝗻 𝗻𝗵𝗮̆́𝗻 : ${storage[rankUser].count}\n[🏅]➜ 𝐑𝐚𝐧𝐤 : ${getRankName(storage[rankUser].count)}\n[⏰]➜ 𝗧𝗶𝗺𝗲: ${timeNow}\n[❓]➜ [ ${global.config.PREFIX}${this.config.name} 𝗵𝗲𝗹𝗽 ] , [ ${global.config.PREFIX}${this.config.name} 𝗮𝗹𝗹 ]\n[🔷]➜ 𝗖𝗵𝘂́𝗰 𝗯𝗮̣𝗻 𝘁𝘂̛𝗼̛𝗻𝗴 𝘁𝗮́𝗰 𝘃𝘂𝗶 𝘃𝗲̉`, attachment: download }, event.threadID, event.messageID);
    }
    return api.sendMessage({
    body: msg, attachment: (await axios.get((await axios.get(`https://docs-api.catteam123.repl.co/images/wallpaper?apikey=JRTvip_2200708248`)).data.data, {
                    responseType: 'stream'
                })).data
}, event.threadID);
