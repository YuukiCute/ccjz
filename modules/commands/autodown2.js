...
exports.handleEvent = async function(o) {
  try {
    ...
    if (/tiktok\.com/.test(a)) {
      ...
      send({
        body: `${head('TIKTOK')}[⚜️] ➜ Tiêu Đề: ${tiktok.title}\n[⚜️] ➜ Lượt Thích: ${tiktok.digg_count}\n[⚜️] ➜ Thời Lượng: ${(tiktok.duration)} Giây\n[⚜️] ➜ Tác Giả: ${tiktok.author.nickname} (${tiktok.author.unique_id})`,
        attachment,
      });
    } else
      if (/(facebook\.com|fb\.watch)/.test(a)) {
      let res = await axios.get(`https://docs-api.catteam123.repl.co/facebook/video?url=${a}`);
      send({
        body: `${head('FACEBOOK')}[⚜️] ➜ Link video facebook: ${res.data.url}\n[⚜️] ➜ Link ảnh: ${res.data.thumbnail}`,
