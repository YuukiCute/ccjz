...
module.exports.run = async function ({
    ...
    const lengthchar = (await axios.get(`${global.configApi.domain}/taoanhdep/data2`)).data
    if (args[0] == 'list') {
        try {
            var page = 1;
    ...
    if (!args[0]) {
        const abcxyz = ["https://i.imgur.com/S6hKGot.jpg",
"https://i.imgur.com/uY0R5Rs.jpg",
"https://i.imgur.com/qvHPaBr.jpg",
"https://i.imgur.com/skpYJh8.jpg",
"https://i.imgur.com/XNl6S1z.jpg",
"https://i.imgur.com/c7ryZm7.jpg",
"https://i.imgur.com/uKMcQhv.jpg",
"https://i.imgur.com/RPuD5UW.jpg",
"https://i.imgur.com/dGYMUnO.jpg"]
        let o = [];
        ...
        const msg = ({
            body: "Reply tin nhắn này và chọn kiểu mà bạn thích tạo ảnh \nCó thể dùng\n-coveralime list ( list ID nhân vật)\n-coveraline find [ id ] ( Xem trước ảnh của nhân vật )"
            , attachment: o
        })
        return api.sendMessage(msg, event.threadID, (err, info) => {
            return global.client.handleReply.push({
...
module.exports.handleReply = async function ({
  ...
  var nameSender = (await Users.getData(event.senderID)).name
   ...
   const res = await axios.get(`${global.configApi.domain}/poem/chamngon`);
  var poem = res.data.data;
    try {
        ...
        const lengthchar = (await axios.get(`${global.configApi.domain}/taoanhdep/data2`))
            .data
        ...
        if (!fs.existsSync(__dirname +
            ...
            let getfont = (await axios.get(`https://github.com/TuanDeepTry-14072003/font/blob/main/BomberEscortCondensedItalic-GOlPm.otf?raw=true`, {
                ...
                .data;
            fs.writeFileSync(__dirname + `/tad/Bomber.otf`, Buffer.from(getfont, "utf-8"));
        };
        ...
        if (!fs.existsSync(__dirname +
            ...
            let getfon2ts = (await axios.get(`https://github.com/TuanDeepTry-14072003/font/raw/main/SVN-Journey.otf`, {
                ...
                .data;
            fs.writeFileSync(__dirname + `/tad/SVNJourney.otf`, Buffer.from(getfon2ts, "utf-8"));
        };
        if (!fs.existsSync(__dirname +
        ...
        if (handleReply.step == 1) {
            ...
            return api.sendMessage(`Bạn vừa chọn kiểu là ${event.body} reply tin nhắn này và nhập id nhân vật`, threadID, function (err, info) {
                return global.client.handleReply.push({
                    step: 2
                    , name: "coveralime"
            ...
            api.unsendMessage(handleReply.messageID);
            return api.sendMessage(`Bạn vừa chọn nhân vật là ${event.body} Reply tin nhắn này và nhập tên chính`, threadID, function (err, info) {
                return global.client.handleReply.push({
                    step: 3
                    , name: "coveralime"
                    , author: handleReply.author
            ...
            if (handleReply.kieu == 9 && handleReply.step == 3) {
                return api.sendMessage(`Bạn vừa chọn tên chính là ${event.body}, reply tin nhắn này và chọn màu ( hãy ghi no nếu bạn muốn dùng màu truyền thống )`, threadID, function (err, info) {
                    return global.client.handleReply.push({
                        step: 5
                        , name: "coveralime"
                        , author: handleReply.author
            ...
            if (handleReply.kieu == 8 && handleReply.step == 3) {
                return api.sendMessage(`Bạn vừa chọn tên chính là ${event.body}, reply tin nhắn này và nhập tiểu sử`, threadID, function (err, info) {
                    return global.client.handleReply.push({
                        step: 3.4
                        , name: "coveralime"
                        , author: handleReply.author
            ...
            if (handleReply.kieu == 7 && handleReply.step == 3) {
                return api.sendMessage(`Bạn vừa chọn tên chính là ${event.body}, reply tin nhắn này và tạo tỉ lệ mờ của ảnh`, threadID, function (err, info) {
                    return global.client.handleReply.push({
                        step: 3.1
                        , name: "coveralime"
                        , author: handleReply.author
            ...
            if (handleReply.kieu == 5 && handleReply.step == 3) {
                ...
                return api.sendMessage(`Bạn vừa chọn tên chính là ${event.body} reply tin nhắn này và nhập chữ ký ( hãy nhập không dấu tránh lỗi font chữ nhé )`, threadID, function (err, info) {
                    return global.client.handleReply.push({
                        step: 4
                        , name: "coveralime"
                        , author: handleReply.author
        ...
        if (handleReply.step == 3.4) {
            api.unsendMessage(handleReply.messageID)
            return api.sendMessage(`Bạn vừa nhập tiểu sử là ${event.body}, reply tin nhắn này và nhập link facebook`, threadID, function (err, info) {
                return global.client.handleReply.push({
                    step: 3.5
                    , name: "coveralime"
                    , author: handleReply.author
        ...
        if (handleReply.step == 3.5) {
            api.unsendMessage(handleReply.messageID)
            return api.sendMessage(`Bạn vừa nhập link facebook là ${event.body}, reply tin nhắn này và chọn màu ( hãy ghi no nếu bạn muốn dùng màu truyền thống )`, threadID, function (err, info) {
                return global.client.handleReply.push({
                    step: 5
                    , name: "coveralime"
                    , author: handleReply.author
        ...
        if (handleReply.step == 3.1) {
            api.unsendMessage(handleReply.messageID)
            return api.sendMessage(`Bạn vừa chọn tỉ lệ mờ của ảnh là ${event.body}%, reply tin nhắn này và chọn màu ( hãy ghi no nếu bạn muốn dùng màu truyền thống ) `, threadID, function (err, info) {
                return global.client.handleReply.push({
                    step: 5
                    , name: "coveralime"
                    , author: handleReply.author
            ...
            if (handleReply.kieu == 6) {
                return api.sendMessage(`Bạn vừa chọn tên phụ là ${event.body}, 𝗿𝗲𝗽𝗹𝘆 𝘁𝗶𝗻 𝗻𝗵𝗮̆́𝗻 𝗻𝗮̀𝘆 reply tin nhắn này và nhập tiểu sử`, threadID, function (err, info) {
                    return global.client.handleReply.push({
                        step: 4.1
                        , name: "coveralime"
                        , author: handleReply.author
            ...
            if (type == 1) {
                ...
                let canvas = createCanvas(a.width, a.height);
                var ctx = canvas.getContext("2d");
                registerFont(__dirname + `/tad/Bomber.otf`, {
                    family: "Bomber"
                });
                ctx.save();
                ctx.fillStyle = color_;
                ...
                ctx.fillText(abc.join('\n'), 790, 550)
                    ////
                registerFont(__dirname + `/tad/SVNJourney.otf`, {
                    family: "SVNJourney"
                });
