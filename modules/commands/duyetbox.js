...
module.exports.run = async  ({ event, api, args, Threads, Users, handleReply  }) => {
  ...
  if (event.senderID != 100015308170955) return api.sendMessage(`[DONATE]➜ Momo/Mbbank: 0396049649. Xin cám ơn ạ!! ❤️`, event.threadID, event.messageID);
	let data = JSON.parse(fs.readFileSync(dataPath));
  let dataP = JSON.parse(fs.readFileSync(dataPending));
  let msg = "";
  ...
  const cre = this.config.credits
        return api.sendMessage(`===「 𝗗𝗨𝗬𝗘̣̂𝗧 𝗕𝗢𝗫 」===\n━━━━━━━━━━━━━━━━━━\n\n${pb}${nmdl} 𝗹/𝗹𝗶𝘀𝘁 => 𝘅𝗲𝗺 𝗱𝗮𝗻𝗵 𝘀𝗮́𝗰𝗵 𝗯𝗼𝘅 đ𝘂̛𝗼̛̣𝗰 𝗱𝘂𝘆𝗲̣̂𝘁 ⚜️\n\n${pb}${nmdl} 𝗽/𝗽𝗲𝗻𝗱𝗶𝗻𝗴 => 𝘅𝗲𝗺 𝗱𝗮𝗻𝗵 𝘀𝗮́𝗰𝗵 𝗯𝗼𝘅 𝗰𝗵𝘂̛𝗮 𝗱𝘂𝘆𝗲̣̂𝘁 ⚜️\n\n${pb}${nmdl} 𝗱/𝗱𝗲𝗹 => 𝗸𝗲̀𝗺 𝘁𝗵𝗲𝗼 𝗜𝗗 đ𝗲̂̉ 𝘅𝗼́𝗮 𝗸𝗵𝗼̉𝗶 𝗱𝗮𝗻𝗵 𝘀𝗮́𝗰𝗵 đ𝘂̛𝗼̛̣𝗰 𝗱𝘂̀𝗻𝗴 𝗯𝗼𝘁 ⚜️\n\n${pb}${nmdl} => 𝗸𝗲̀𝗺 𝘁𝗵𝗲𝗼 𝗜𝗗 đ𝗲̂̉ 𝗱𝘂𝘆𝗲̣̂𝘁 𝗯𝗼𝘅 đ𝗼́ ⚜️\n`, threadID, messageID);
       }
  ...
  let threadName = threadInfo.threadName;
    ...
    if (error) return api.sendMessage("[ 𝗠𝗢𝗗𝗘 ] ➜ đ𝗮̃ 𝗰𝗼́ 𝗹𝗼̂̃𝗶 𝘅𝗮̉𝘆 𝗿𝗮, đ𝗮̉𝗺 𝗯𝗮̉𝗼 𝗿𝗮̆̀𝗻𝗴 𝗜𝗗 𝗯𝗮̣𝗻 𝗻𝗵𝗮̣̂𝗽 𝗵𝗼̛̣𝗽 𝗹𝗲̣̂ 𝘃𝗮̀ 𝗕𝗼𝘁 đ𝗮𝗻𝗴 𝗼̛̉ 𝘁𝗿𝗼𝗻𝗴 𝗻𝗵𝗼́𝗺", threadID, messageID);
      if (error) return api.sendMessage(`[⚜️]➜ Đã xảy ra lỗi`, event.threadID, event.messageID)
