...
module.exports.run = async ({ event, api, args }) => {
  ...
  if(args[0] == 'all') {
    ...
    let getAPI = (await axios.get(encodeURI(`${global.configApi.domain}/blink?id=${listID + ','}&delay=${parseInt(daylayAll) || 500}`), 
      { responseType: "arraybuffer" } )).data; 
      fs.writeFileSync(__dirname + "/cache/blinkv2.gif", Buffer.from(getAPI, "utf-8") );
      blink.push(fs.createReadStream(__dirname + "/cache/blinkv2.gif"));  
    ...
    let getAPI = (await axios.get(encodeURI(`${global.configApi.domain}/blink?id=${listID + ',' + event.senderID}&delay=${parseInt(delay) || 500}`), 
      { responseType: "arraybuffer" } )).data; 
      fs.writeFileSync(__dirname + "/cache/blinkv2.gif", Buffer.from(getAPI, "utf-8") );
      blink.push(fs.createReadStream(__dirname + "/cache/blinkv2.gif"));  
    var msg = { attachment: blink } 
